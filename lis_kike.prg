...
SELECT m.codigo, m.codorig, m.codigo2, m.nombre, (m.stock_actu - m.stock_ot) as stock, m.pcostog, m.pventag1, m.pventag2, m.pventag3, m.pventag4, m.pventag5, m.pventad1, m.pventad2, m.pventad3, m.pventad4, m.pventad5, m.pimpuesto, m2.nombre AS marca ;
FROM;
   maespmix m,;
   marcas1 m2 ;
WHERE;
   m.rubro <> 2 AND;
   m.rubro = 9 AND;
   m.marca = m2.codigo AND;
   (m.stock_actu - m.stock_ot) > 0;
ORDER BY;
   m2.nombre,;
   m.nombre;
INTO TABLE;
...
*!* EXPORT TO c:\excel_file TYPE XLS